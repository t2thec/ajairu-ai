...
<table width="100%" cellpadding="0" cellspacing="0" border="0" style="background:${BRAND.accentLight};border-radius:10px;border:1px solid ${BRAND.accent};margin-top:24px;">
  <tr>
    <td style="padding:24px;text-align:center;">
      ...
      <p style="margin:0 0 16px;font-family:Arial,sans-serif;font-size:14px;color:#3d4d5c;line-height:1.5;">Our AI Opportunity Audit (from &pound;750) examines your actual operations and gives you a prioritised report with ROI estimates.</p>
  ...
  await transporter.sendMail({
    ...
    text: `Your AI Readiness Score: ${score}/100\n\n${verdict}\n\nBreakdown:\n${categories.map(c => `${c.name}: ${c.score}/100`).join('\n')}\n\nWant to go deeper? Our AI Opportunity Audit (from £750) examines your actual operations and gives you a prioritised report with ROI estimates. Book a free call: ${BRAND.contactUrl}\n\n${emailConfig.fromName}`,
    html: wrapEmailHtml(innerHtml, { title: `Your AI Readiness Score: ${score}/100`, preheader: `Your score is ${score}/100` }),
  });
}
...
async function sendGuideEmail(toEmail) {
  ...
  const guideTitle = '5 AI Use Cases That Pay for Themselves in 90 Days';
  const guideSummary = `<p style="margin:0 0 16px;font-family:Arial,sans-serif;font-size:15px;color:#334155;line-height:1.6;">Here is a quick summary of the five use cases covered in the guide:</p>
<ol style="margin:0 0 16px;padding-left:20px;font-family:Arial,sans-serif;font-size:14px;color:#334155;line-height:1.8;">
<li><strong>Customer Enquiry Triage and Response</strong> &ndash; 60-80% of enquiries get a first response in seconds</li>
<li><strong>Invoice and Document Processing</strong> &ndash; 70% reduction in processing time</li>
<li><strong>Meeting Notes and Action Item Capture</strong> &ndash; 3-5 hours saved per person per week</li>
<li><strong>Content Drafting for Marketing</strong> &ndash; 50-70% reduction in content creation time</li>
<li><strong>Internal Knowledge Base and Search</strong> &ndash; 50% reduction in time spent searching</li>
</ol>`;

  // PDF attachment path
  const pdfPath = path.join(__dirname, 'public', 'guides', '5-ai-use-cases-that-pay-for-themselves.pdf');
  let attachments = [];
  if (fs.existsSync(pdfPath)) {
    attachments.push({
      filename: '5-ai-use-cases-that-pay-for-themselves.pdf',
      path: pdfPath,
